...
export async function buildApp(options: BuildOptions = {}): Promise<FastifyInstance> {
  ...
  const db = new Database(options.dbPath ?? process.env.DATABASE_PATH ?? "relay-chat.sqlite");
  ...
  app.post<{ Body: { body: string } }>(
    "/messages", { preHandler: authenticate, schema: MESSAGE_SCHEMA }, async (request, reply) => {
      const github = await enrich(client, request.body.body);
